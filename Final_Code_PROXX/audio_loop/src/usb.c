...
static char *USB_FormatString(char *message, int length) {
	...
	char *output = new char[length/2]; // modified to work for integration with C++ project
	if (output == NULL) {
...
static void XUsbPs_QHInit(XUsbPs_HostConfig *HostCfgPtr)
{
	...
	for (QHNum = 0; QHNum < HostCfgPtr->NumQHs; ++QHNum) {
		if (QHNum == 0) {			// Temp config: EPS=FS EpNum=0 Addr=0 H=1 (Head) DTC=1
			...
			XUsbPs_WritedQH(QueueHead[QHNum].pQH, XUSBPS_QHCFG0, 0xF800000C);
					...
					u16 UsbDeviceId, u16 UsbIntrId, void *UsbIntrHandler)
{
	...
	HostConfig.QueueHead[1].BufSize			= 64;
	HostConfig.QueueHead[1].MaxPacketSize	= 64;
	...
	Status = XUsbPs_IntrSetHandler(UsbInstancePtr, reinterpret_cast<XUsbPs_IntrHandlerFunc>(UsbIntrHandler), NULL, XUSBPS_IXR_UI_MASK | XUSBPS_IXR_PC_MASK);
	if (XST_SUCCESS != Status) {
		return ReturnStatus;
	}
